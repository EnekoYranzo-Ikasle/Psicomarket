...
const categoriasSeleccionadas = document.getElementById('categorias-seleccionadas');
...
function abrirSelect() {
  abrir.addEventListener('click', (e) => {
    e.stopPropagation(); // evita que el click propague al document y cierre inmediatamente
...
function abrirFiltros() {
  ...
  function cerrarFiltros(e) {
    if (!filtroContainer.contains(e.target) && !toggleFiltros.contains(e.target)) {
...
function abrirSelectCategorias() {
  document.addEventListener('click', (e) => {
    ...
    if (estaAbierto && !dropdown.contains(e.target) && !abrir.contains(e.target)) {
