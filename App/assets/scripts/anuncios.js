...
async function obtenerAnuncios() {
  try {
    ...
    if (esPaginaMisComercios) {
      ...
      if (filtros.cantidadProductos) q.append('cantidadProductos', filtros.cantidadProductos);
    ...
    showError('Error cargando anuncios:', error);
...
function getFiltrosSeleccionados() {
  ...
  const catChecks = document.querySelectorAll('#lista-categorias input[type="checkbox"]:checked');
